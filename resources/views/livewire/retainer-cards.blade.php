<div class="py-12">
    <div class="grid grid-cols-1 gap-5 mx-auto lg:grid-cols-3 max-w-7xl sm:px-6 lg:px-8">
        @foreach ($retainers as $retainer)
            <div wire:key="{{ $retainer->id }}" class="overflow-hidden bg-white shadow-sm dark:bg-zinc-800 sm:rounded-lg">
                <div class="p-6 text-lg text-zinc-900 dark:text-zinc-100">
                    <div class="flex items-center justify-between">
                        <div>{{ $retainer->name }}<span
                                class="ml-2 text-xs dark:text-zinc-100">[{{ $retainer->server }}]</span></div>
